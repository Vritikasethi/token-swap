...
async function main() {
  ...
  const ERC20RBNT = await  hre.ethers.getContractFactory("ERC20");

  const contract1 = await ERC20.deploy();
  const contract2 = await ERC20RBNT.deploy();
  ...
  const contract3 = await POOL.deploy(await contract1.getAddress(), await contract2.getAddress());
